...
log.prototype._push = function(level,data){
	...
	if(messagePriority.indexOf(level) >= messagePriority.indexOf(this.consoleLevel)){
		console.log(moment(new Date()).format(this.timeStampFormat)+" ["+level+"] "+msg)
	}
		
	this.pool.push({
		logger: this.id,
		scope: this.scope,
		timeStamp: 	moment(new Date()).format(this.timeStampFormat),
		level: 		level,
		message:   	msg     
	})	
}
...
log.prototype.merge = function(log){
	...
	this.pool.sort( (a,b) => moment(new Date(a)).isBefore(moment(new Date(b))))
    return this;
};
