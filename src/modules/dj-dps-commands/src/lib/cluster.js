let cosin_similarity = require("compute-cosine-similarity")


  function compareArray(a1, testArr) {
    if (a1.length != testArr.length) return false;
    for (var i = 0; i < testArr.length; i++) {
      if (Array.isArray(a1[i])) {
        if (!compareArray(a1[i], testArr[i])) return false;
      }
      if (a1[i] !== testArr[i]) return false;
    }
    return true;
  }
  ...
  function cosinSimilarity(vec1, vec2) {
  ...
  function agglomerate(labels, vectors, distance, linkage) {
    ...
    else if (distance == figue.COSIN_SIMILARITY)
      distance = cosinSimilarity;
  ...
  function kmeans(k, vectors, distance, initialPosition) {
    ...
    while (repeat) {
      ...
      repeat = false;
      for (var j = 0; j < k; j++) {
        if (!compareArray(newCentroids[j], centroids[j])) {
          repeat = true;
          break;
        }
      }
    ...
    while (repeat) {
      ...
      if (centroids == null) {
        ...
        for (var j = 0; j < k; j++) {
          centroids[j] = [];
          norm = 0;
          for (var i = 0; i < membershipMatrix.rows; i++) {
      ...
      diff;
      for (var i = 0; i < membershipMatrix.rows; i++)
        for (var j = 0; j < membershipMatrix.cols; j++) {
